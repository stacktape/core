...
archive_source_url=https://github.com/stacktape/core/releases/download/$version/macos-arm.tar.gz
...
if command -v stacktape >/dev/null; then
  ...
  info_bold "To use 'stacktape' and 'stp' aliases, you need to add them to \$PATH. To do that, run:\n"
  info_bold "  sudo touch $etc_paths_path && echo $bin_dir_path | sudo tee $etc_paths_path\n"
